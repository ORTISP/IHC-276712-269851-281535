const validation = {
  ...
  isValidDateOfBirth: (dateOfBirth) => {
    ...
    if (isNaN(date.getTime())) {
      return { isValid: false, error: 'Please provide a valid date of birth (YYYY-MM-DD format)' };
    }
  ...
  isValidGender: (gender) => {
    if (!gender || typeof gender !== 'string') {
      return { isValid: false, error: 'Gender is required and must be a string' };
    }
    ...
    if (!validGenders.includes(normalizedGender)) {
      return { isValid: false, error: 'Gender must be one of: male, female, other, prefer-not-to-say' };
    }
  ...
  validateUserUpdate: (updateData) => {
    ...
    if (updateData.dateOfBirth !== undefined) {
      const dobValidation = validation.isValidDateOfBirth(updateData.dateOfBirth);
      if (!dobValidation.isValid) {
        errors.push(dobValidation.error);
      } else {
        sanitizedData.dateOfBirth = dobValidation.sanitized;

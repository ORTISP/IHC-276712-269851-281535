...
const MultiSelectField = ({ label, value = [], options = [], onChange, placeholder = 'Seleccionar...', className, style }) => {
  ...
  const getSelectedLabels = () => {
    ...
    if (value.length === 1) {
      const option = options.find(opt => (opt.value !== undefined ? opt.value : opt) === value[0]);
  ...
  return (
    <View className={`mb-4 ${className || ''}`} style={style}>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Text className={`text-base flex-1 ${value.length === 0 ? 'text-gray-500' : 'text-gray-900'}`}>
          {getSelectedLabels()}
        </Text>
